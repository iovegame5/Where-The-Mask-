...
def mask():
    ...
    cv2.putText(bgr_image, 'Press "Q" to exit', (350, 475),
                cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 0), 2, cv2.LINE_4)
    for face in faces:
        x, y, w, h = face
        ...
        print(prediction)
        if masked:
            cv2.putText(bgr_image, 'Masked', (x, y-10),
                        cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 255, 0), 2)
            cv2.rectangle(bgr_image, (x, y), (x+w, y+h), (0, 255, 0), 5)

        elif unmasked:
            cv2.putText(bgr_image, 'Unmasked', (x, y-10),
